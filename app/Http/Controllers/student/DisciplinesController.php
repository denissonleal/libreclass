...
class DisciplinesController extends Controller
{
  ...
  public function getIndex()
    ...
    $offers= DB::select("SELECT Offers.id, Disciplines.name as discipline FROM Attends, Units, Offers, Disciplines "
                  . " WHERE Attends.user_id=? AND Attends.unit_id=Units.id AND Units.offer_id=Offers.id AND Offers.discipline_id=Disciplines.id", [auth()->id()]);
