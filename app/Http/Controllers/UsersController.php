...
class UsersController extends Controller
{
	...
	public function postSearchTeacher()
	{
		...
		if ($teacher) {
			$relationship = Relationship::where('user_id', auth()->id())->where('friend_id', $teacher->id)->first();
			if (!$relationship) {
	...
	public function postTeacher()
	{
		...
		if (strlen(request()->get("teacher"))) {
			...
			if (strlen(request()->get("registered"))) {
				$relationship = Relationship::where('user_id', auth()->id())->where('friend_id', $user->id)->first();
				if (!$relationship) {
	...
	public function getStudent()
	{
		...
		$users = $users->skip($current * $block)
			->take($block)
			->get(['name', 'enrollment']);
	...
	public function printScholarReport()
	{
		...
		$e = Relationship::where('user_id', auth()->id())->where('friend_id', $data['student']->id)->first();
		...
		$disciplines = DB::select("
			...
			WHERE
				...
				and Courses.id = ?
			GROUP BY Offers.id",
			[auth()->id(), request()->get('school_year'), $data['student']->id, implode(',', request()->get('unit_value')), request()->get('course')]
		);
